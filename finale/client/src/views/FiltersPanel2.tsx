...
const FiltersPanel2: FC<{ filters: FiltersState, name: string }> = ({ filters, name }) => {
                ...
                data.metadata.forEach((el : any)=>{
                    ...
                    if(sm2.country.set || sm2.region.set){
                        if(sm2.country.list.includes(el.country) || sm2.country.list.includes(el.division) || sm2.country.list.includes(el.originating_lab) || sm2.region.list.includes(el.region)){
                            sampleFound.push(el.gisaid_epi_isl);
                            //sampleFound.push(el.epi_id);
                        }
                    //altrimenti vengono cercate tutte
                    }else sampleFound.push(el.gisaid_epi_isl);
                    //vengono eliminate le mutation che non contengono i giusti sex o age
                    if(sm2.sex.set){
                        if (el.sex && !sm2.sex.list.includes(el.sex.toLowerCase())){
                            const index = sampleFound.indexOf(el.gisaid_epi_isl);
    ...
    async function getGraph() {
        ...
        const res = await fetch(`${process.env.PUBLIC_URL}/available/${name}`)
        ...
        metadata.forEach((meta: any) => {
            if (epiList.length){
                if (epiList.includes(meta.gisaid_epi_isl)){
                    content+=`W\t${meta.gisaid_epi_isl}\t0\tNC_045512.2\t0\t29903\t${meta.path}\n`
                    cont++;
                }
            }else if (ref===false) content+=`W\t${meta.gisaid_epi_isl}\t0\tNC_045512.2\t0\t29903\t${meta.path}\n`
        })
        return content
    }

    const downloadVision = async (graphName: string) => {
        try {
            const gra = await getGraph()
            axios.post('/vision', { name: graphName, graph: gra})
                .then(response => {
                    let file = new Blob([response.data], {type: 'text/plain'});
                    let fileURL = URL.createObjectURL(file);
                    let a = document.createElement('a');
                    a.href = fileURL;
                    a.download = 'downloaded_graph.gfa';
                    a.click();
                })
            ...
            graph.forEachEdge((key) =>{
                ...
                if (graph.getEdgeAttribute(key, 'is_ref') === 'false'){
                    if( graph.getEdgeAttribute(key, 'sample') === '') {
                        ...
                        for (let i=0; i < epiList.length; i++) { 
                            if(graph.getEdgeAttribute(key, 'sample') && graph.getEdgeAttribute(key, 'sample').includes(epiList[i])){
    ...
    return (
      ...
      >
            ...
            <button className='btn' onClick={()=>downloadVision(name)}>Download GFA</button> 
       </Panel>
    );
};
