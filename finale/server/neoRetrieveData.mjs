...
const path_write = '../client/public/available/sars-cov-2.json'
...
await session1.run(queryNode).then(result=> {
    ...
    fs.writeFile(path_write, content, err => {
        if (err) {
          console.error(err)
          return
        }
...
await session2.run(queryEdge).then(result => {
    ...
    result.records.forEach(record=>{
        ...
        content+=`\t\t["${record.toObject()['l.from']}","${record.toObject()['l.to']}","${record.toObject()['l.from_is_reversed']}","${record.toObject()['l.to_is_reversed']}","${sample.includes('reference')}","${sample}"]${end}`
        i++;
    })
    fs.appendFile(path_write, content, err => {
        if (err) {
          console.error(err)
          return
        }
