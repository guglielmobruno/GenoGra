...
export function drawHover(context: CanvasRenderingContext2D, data: PlainObject, settings: PlainObject) {
  ...
  if(sampleLabel && sampleLabel[0]==='reference') start=1;
  ...
  if (sampleLabel) hSampleLabel = (sampleLabel && sampleLabel?.length===1 && sampleLabel[0]==='reference') ? 0 : fieldInterline + (subLabelSize + interline)*(Math.floor(sampleLabel.length/3)+1);
  else hSampleLabel= 0;
  let leng=0;
  if (sampleLabel ) leng = sampleLabel.length
  let hSubLabel = fieldInterline + (subLabelSize + interline)*(Math.floor(context.measureText(subLabel).width/MAX_WIDTH)+1);
  const toShort = (Math.floor(leng/3)+1) > (Math.floor(context.measureText(subLabel).width/MAX_WIDTH)+1) ? 'Samp' : 'Seq';
